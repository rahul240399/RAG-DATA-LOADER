...
class TestDocumentProcessingResultConsistency:
    ...
    def test_success_with_storage_failure_invalid(self):
        """Test that success=True with storage_success=False and embeddings > 0 raises ValueError."""
        with pytest.raises(ValueError, match="Overall success cannot be True if storage failed with embeddings generated"):
            DocumentProcessingResult(
                success=True,
                chunks_processed=10,
                embeddings_generated=10,
...
class TestBatchProcessingResultBasic:
    ...
    def test_combine_results_method(self):
        ...
        results = [
            DocumentProcessingResult(
                success=True, chunks_processed=10, embeddings_generated=10,
                storage_success=True, processing_time=5.0, errors=[], source_document="doc1.pdf"
            ),
            DocumentProcessingResult(
                success=False, chunks_processed=5, embeddings_generated=3,
                storage_success=False, processing_time=2.0, errors=["Error"], source_document="doc2.pdf"
            )
        ]
...
class TestBatchProcessingResultMethods:
    ...
    def test_get_success_rate_mixed_results(self):
        ...
        for i in range(3):
            batch.add_document_result(DocumentProcessingResult(
                success=True, chunks_processed=10, embeddings_generated=10,
                storage_success=True, processing_time=5.0, errors=[], source_document=f"success{i}.pdf"
            ))
        
        for i in range(2):
            batch.add_document_result(DocumentProcessingResult(
                success=False, chunks_processed=5, embeddings_generated=0,
                storage_success=False, processing_time=2.0, errors=["Error"], source_document=f"failed{i}.pdf"
            ))
    ...
    def test_get_embedding_success_rate(self):
        ...
        batch.add_document_result(DocumentProcessingResult(
            success=True, chunks_processed=10, embeddings_generated=8,
            storage_success=True, processing_time=5.0, errors=[], source_document="doc1.pdf"
        ))
        
        batch.add_document_result(DocumentProcessingResult(
            success=False, chunks_processed=10, embeddings_generated=2,
            storage_success=False, processing_time=3.0, errors=["Error"], source_document="doc2.pdf"
        ))
    ...
    def test_get_failed_documents(self):
        ...
        success_result = DocumentProcessingResult(
            success=True, chunks_processed=10, embeddings_generated=10,
            storage_success=True, processing_time=5.0, errors=[], source_document="success.pdf"
        )
        
        failed_result = DocumentProcessingResult(
            success=False, chunks_processed=5, embeddings_generated=0,
            storage_success=False, processing_time=2.0, errors=["Error"], source_document="failed.pdf"
        )
    ...
    def test_get_all_errors(self):
        ...
        batch.add_document_result(DocumentProcessingResult(
            success=False, chunks_processed=5, embeddings_generated=0,
            storage_success=False, processing_time=2.0, 
            errors=["Doc error 1", "Doc error 2"], source_document="failed.pdf"
        ))
...
class TestBatchProcessingResultEdgeCases:
    ...
    def test_get_summary_statistics_with_data(self):
        ...
        batch.add_document_result(DocumentProcessingResult(
            success=True, chunks_processed=10, embeddings_generated=10,
            storage_success=True, processing_time=3.0, errors=[], source_document="doc1.pdf"
        ))
        
        batch.add_document_result(DocumentProcessingResult(
            success=False, chunks_processed=5, embeddings_generated=2,
            storage_success=False, processing_time=2.0, errors=["Error1", "Error2"], source_document="doc2.pdf"
        ))
