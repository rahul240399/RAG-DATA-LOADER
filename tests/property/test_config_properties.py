...
class TestPipelineConfigProperties:
    ...
    def test_configuration_parameter_validation(self, chunk_size, chunk_overlap, 
                                              embedding_model, chromadb_collection,
        ...
        **Validates: Requirements 5.1, 5.2, 5.3**
        ...
        should_be_valid = (
            chunk_size > 0 and  # chunk_size must be positive
            chunk_overlap >= 0 and  # chunk_overlap must be non-negative
            chunk_overlap < chunk_size and  # chunk_overlap must be less than chunk_size
            isinstance(embedding_model, str) and len(embedding_model.strip()) > 0 and  # embedding_model must be non-empty string
            isinstance(chromadb_collection, str) and len(chromadb_collection.strip()) > 0 and  # chromadb_collection must be non-empty string
            isinstance(chromadb_host, str) and len(chromadb_host.strip()) > 0 and  # chromadb_host must be non-empty string
            isinstance(chromadb_port, int) and 1 <= chromadb_port <= 65535  # chromadb_port must be in valid port range
        )
        
        # Property: Validation result should match expected validity
        assert validation_result == should_be_valid, (
            f"Validation result {validation_result} does not match expected validity {should_be_valid} "
    ...
    @settings(max_examples=100)
    def test_valid_configurations_pass_validation(self, chunk_size, chunk_overlap,
                                                embedding_model, chromadb_collection,
                                                chromadb_host, chromadb_port):
        """
    ...
    def test_default_configuration_is_valid(self):
        ...
        assert default_config.validate() == True, (
            f"Default configuration should be valid: {default_config}"
        )
    ...
    def test_chunk_size_and_overlap_validation(self, chunk_size, chunk_overlap):
        ...
        expected_valid = (
            chunk_size > 0 and  # chunk_size must be positive
            chunk_overlap >= 0 and  # chunk_overlap must be non-negative
            chunk_overlap < chunk_size  # chunk_overlap must be less than chunk_size
        )
