...
class DocumentProcessingResult:
    ...
    def _validate_consistency(self) -> None:
        ...
        if self.success and self.embeddings_generated > 0 and not self.storage_success:
            raise ValueError("Overall success cannot be True if storage failed with embeddings generated")
    ...
    def is_partial_success(self) -> bool:
        ...
        """
        return (
            self.chunks_processed > 0 and 
            self.embeddings_generated > 0 and 
            self.embeddings_generated < self.chunks_processed
        )
...
class BatchProcessingResult:
        ...
        for result in self.document_results:
            if result.errors:
                document_errors.extend([
                    f"{result.source_document}: {error}" 
    ...
    def is_complete_success(self) -> bool:
        ...
        return (
            ...
            self.failed_documents == 0 and
            len(self.batch_errors) == 0
        )
    ...
    def is_partial_success(self) -> bool:
        ...
        return (
            self.total_documents > 0 and
            self.successful_documents > 0 and
            self.failed_documents > 0 and
            len(self.batch_errors) == 0
        )