...
class TestTextChunkProperties:
    ...
    def test_chunk_id_generation_consistency(
        self, content, metadata, start_index, end_index, source_document
    ):
        """
        **Feature: rag-indexing-pipeline, Property 1: Chunk ID generation consistency**
        ...
        assert md5_pattern.match(chunk1.chunk_id), (
            f"Chunk ID should be a valid MD5 hash (32 hex characters). Got {chunk1.chunk_id}"
        )
    ...
    @settings(max_examples=100)
    def test_chunk_id_uniqueness_for_different_positions(
        self, content, metadata, start_index1, end_index1, start_index2, end_index2, source_document
    ):
        """
