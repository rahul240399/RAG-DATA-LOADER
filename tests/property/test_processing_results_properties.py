...
class TestProcessingResultsProperties:
    ...
    def test_resource_management_and_reporting(
        self,
        success,
        chunks_processed,
        ...
        **Validates: Requirements 6.4, 6.5**
        ...
        try:
            ...
            expected_partial = (
                ...
                and result.embeddings_generated < result.chunks_processed
            )
    ...
    @given(data=st.data(), batch_errors=st.lists(st.text(min_size=1, max_size=100), max_size=5))
    @settings(max_examples=100)
    def test_batch_processing_resource_management(self, data, batch_errors):
        ...
        expected_doc_success_rate = (
            expected_successful / len(document_results) if len(document_results) > 0 else 0.0
        )
        expected_embedding_success_rate = (
            expected_total_embeddings / expected_total_chunks if expected_total_chunks > 0 else 0.0
        )
        ...
        assert (
            abs(batch_result.get_embedding_success_rate() - expected_embedding_success_rate) < 1e-10
        ), (
        ...
        expected_complete_success = (
            len(document_results) > 0 and all_docs_successful and not has_batch_errors
        )
        expected_complete_failure = (
            len(document_results) > 0 and no_docs_successful
        ) or has_batch_errors
        expected_partial_success = (
            len(document_results) > 0
            and not all_docs_successful
            and not no_docs_successful
            and not has_batch_errors
        )
    ...
    @settings(max_examples=100)
    def test_error_handling_resource_cleanup(
        self,
        initial_success,
        initial_chunks,
        initial_embeddings,
        initial_storage_success,
        initial_time,
        initial_errors,
        source_doc,
        error_to_add,
        ...
        try:
            ...
            assert result.chunks_processed == initial_chunks, (
                "Chunks processed should remain unchanged after adding error"
            )
            assert result.embeddings_generated == initial_embeddings, (
                "Embeddings generated should remain unchanged after adding error"
            )
            assert result.storage_success == adjusted_storage_success, (
                "Storage success should remain unchanged after adding error"
            )
            assert abs(result.processing_time - initial_time) < 1e-10, (
                "Processing time should remain unchanged after adding error"
            )
            assert result.source_document == source_doc, (
                "Source document should remain unchanged after adding error"
            )
